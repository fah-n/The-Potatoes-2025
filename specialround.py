...
def placedownfinal(): # leftmost donut position
    ...
    movespeed(3, 95, 75, 0.5) #angle outwards
...
def donut_bearing_and_color(frame, model, hfov_deg, conf_thresh, target_color=None):
    """
    If target_color is provided ('r','g','b','y','p'), the chosen target is
    the RIGHTMOST detection of that color. Otherwise, it falls back to your
    original behavior (rightmost of any label).
    """
    H,W = frame.shape[:2]
    cx_frame = W//2
    pred = model(frame)[0]
    ...
    # pick rightmost one (optionally filtered by target_color)
    candidates = [i for i,d in enumerate(detections) if (target_color is None or d['lab'] == target_color)]
    if not candidates:
        return None, None, detections, None

    target_idx = max(candidates, key=lambda i: detections[i]['cx'])
...
def turn_until_align(
    ...
    frame_size=(640,360),
    max_runtime=25.0,
    last_color_hint='u',   # seed the memory with a previous/expected color
    target_color=None      # NEW: only align to this color if provided ('r','g','b')
):
    ...
    # memory of last seen (non-'u') color for the CURRENT frame
    ...
    while time.time()-t0 < max_runtime:
        ...
        # Donut bearing + color + all detections (NOW can filter for target_color)
        donut_bearing, color, dets, target_idx = donut_bearing_and_color(
            frame, model, hfov_deg, conf_thresh, target_color=target_color
        )
        ...
        for i, d in enumerate(dets):
            ...
            thickness = 3 if (target_idx is not None and i == target_idx) else 1
            cv2.rectangle(frame, (x1,y1), (x2,y2), col, thickness)
            cv2.putText(frame, lab, (x1, max(0,y1-8)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, col, 2)

        # Update memory if we saw a non-'u' color (should match target_color when locked)
        ...
        want     = "-" if target_color is None else target_color
        cv2.putText(frame, f"want:{want}  yaw:{txt_yaw}  donut:{txt_bear}  last:{last_seen_color}",
...
def main():
    try:
        ...
        # RED first
        angle, color = turn_until_align(target_color='r', last_color_hint='r')
        currentangle = int(angle)
        print(f"[RED] Stopped at {currentangle}, color={color}")
        if color == 'r':
            pickup(currentangle)
        else:
            print("[RED] Not found/aligned, skipping pickup.")

        # GREEN next
        angle, color = turn_until_align(target_color='g', start_angle_deg=currentangle, last_color_hint='g')
        currentangle = int(angle)
        print(f"[GREEN] Stopped at {currentangle}, color={color}")
        if color == 'g':
            pickup(currentangle)
        else:
            print("[GREEN] Not found/aligned, skipping pickup.")

        # BLUE last
        angle, color = turn_until_align(target_color='b', start_angle_deg=currentangle, last_color_hint='b')
        currentangle = int(angle)
        print(f"[BLUE] Stopped at {currentangle}, color={color}")
        if color == 'b':
            pickup(currentangle)
        else:
            print("[BLUE] Not found/aligned, skipping pickup.")
        

    except KeyboardInterrupt:
        print("\nProgram interrupted by user (Ctrl+C). Shutting down...")
    finally:
        board.exit()  # close port safely
